    ...
    while(!tmp.empty()){
        ...
        while(next!=0){
          ...
          //  next=m[next][tmp.front()].node_d;
              next = m[tmp.front()][next].node_r;
        }
        tmp.pop();
    }
    if(can_get[dest]==0)  return false;
    ...
    for(int i=0;i<=total_num;i++)
    {
        ...
        for(int j=0;j<ant_num;j++)  //依次让每只蚂蚁去走
        {
            for(int k=0;k<50;k++)   //每只蚂蚁走50步
            {
				...
				int next=m[antlist[j].location][0].node_r;
                ...
                while(next!=0)
                {
                    ...
                    next=m[antlist[j].location][next].node_r;
                }
                ...
                for(int n=0;n<nextlist.size();n++)//依次查看接下来的点
                {
                    ...
                    if(isvia[nextlist[n]]==1)//如过是必经节点
                    {
                        ...
                        //if(antlist[j].path_len+m[nextlist[n]][antlist[j].location].length<history[nextlist[n]].min_len)
							if(antlist[j].path_len+m[antlist[j].location][nextlist[n]].length<history[nextlist[n]].min_len)
                        {
                            if(is_available(nextlist[n],antlist[j].path))//如果此路径可行则替换掉原来的路径
                            {
                                history[nextlist[n]].min_len=antlist[j].path_len+m[antlist[j].location][nextlist[n]].length;
                                history[nextlist[n]].min_len_path=antlist[j].path;
                                history[nextlist[n]].minlen_via_num=antlist[j].via_num+1;
                            }
                        }else if(antlist[j].path_len+m[antlist[j].location][nextlist[n]].length<history[nextlist[n]].min_len+50)
                        { //如果不是最优但接近最有(尽超过50)则将其存储在此必经点的可到达路径中，并将该路径长度加入到长度列表里
                             history[nextlist[n]].can_get_path.push_back(antlist[j].path);
                             history[nextlist[n]].get_lenlist.push_back(antlist[j].path_len+m[antlist[j].location][nextlist[n]].length);
                        }
                    ...
                    }else if(nextlist[n]==dest)
                    {
                        if (antlist[j].via_num==designatedN)
                            ...
                            key*=0.1;
                    }else if(m[nextlist[n]][0].node_r==0)
                        key = 0.0;
